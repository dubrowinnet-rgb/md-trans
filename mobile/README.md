...
- `(dispatcher)/` — вкладки диспетчера: `calendar` (режимы 1, 3 и 7 дней;
  листается бесконечно и всегда останавливается на границе страницы; при
  входе открывается на красной линии текущего времени; заказ окрашен в цвет
  своей услуги, закончившийся — в серый; фильтр по сотруднику, «+» и тап по
  слоту),
  `clients` (поиск, телефон, персональная скидка, заметки; «Добавить»
  сразу открывает контакты телефона и подставляет имя и номер) и `employees`
  (список и добавление).
...
- `order/[id].tsx` — карточка заказа: диспетчер меняет статус; сотрудник при
  ...
  позвонить клиенту. Доп. точки маршрута свёрнуты под «Ещё точки»; кнопка
  «Маршрут в Яндекс.Картах» строит маршрут по всем точкам.
- `order/new.tsx` — создание заказа: клиент обязателен (поиск или новый из
  контактов телефона),
