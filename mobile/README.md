...
- `(dispatcher)/` — вкладки диспетчера: `calendar` (в режиме «День» —
  колонка на каждого сотрудника, в «Неделе» — колонка на день; фильтр по
  сотруднику, «+» и тап по слоту, из колонки сотрудника он сразу назначается),
