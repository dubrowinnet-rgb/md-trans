...
// компаниям сразу. Доступ временно проверяется как role==='admin'
// (см. lib/ownerAccess.ts) — сегодня это безопасно, потому что
// администратор в системе один — сам Максим.
export default function OwnerPage() {
  ...
  const companiesMissing = companiesQuery.data?.missingTable ?? false;
  const tickets = ticketsQuery.data?.tickets ?? [];
  const ticketsMissing = ticketsQuery.data?.missingTable ?? false;
    ...
    <Box p="lg">
      ...
      <Tabs defaultValue="companies">
        ...
        <Tabs.Panel value="companies">
          {companiesMissing ? (
            <ComingSoon text="Список компаний появится, как только будет готова база для нескольких тенантов." />
          ) : (
            <>
        ...
        <Tabs.Panel value="tickets">
          {ticketsMissing ? (
            <ComingSoon text="Очередь обращений появится вместе с остальным кабинетом владельца." />
          ) : (
            <>
